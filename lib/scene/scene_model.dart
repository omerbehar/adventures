...
final class MeterSpec {
  ...
  factory MeterSpec.fromJson(Map<String, Object?> j) => MeterSpec(
    min: j['min'] as int,
    max: j['max'] as int,
    initial: j['initial'] as int,
  );
}
...
sealed class ThresholdExpr {
  ...
  factory ThresholdExpr.fromJson(Object? node) {
    if (node is! Map) {
      throw SceneParseException('ThresholdExpr must be an object: $node');
    }
    ...
    switch (entry.key) {
      ...
      case 'IfFacet':
        ...
        return IfFacet(
          a[0] as String,
          ThresholdExpr.fromJson(a[1]),
          ThresholdExpr.fromJson(a[2]),
        );
      case 'WorldFacet':
...
final class SolutionPath {
  ...
  factory SolutionPath.fromJson(Map<String, Object?> j) => SolutionPath(
    ...
    difficulty: j['difficulty'] == null
        ? null
        : Difficulty.values.byName(j['difficulty'] as String),
  );
}
...
final class ReactiveThreshold {
  ...
  factory ReactiveThreshold.fromJson(Map<String, Object?> j) =>
      ReactiveThreshold(
        id: j['id'] as String,
...
final class FallbackBounds {
  ...
  factory FallbackBounds.fromJson(Map<String, Object?> j) => FallbackBounds(
    touchableFacets: {
      ...(j['touchableFacets'] as List? ?? const []).cast<String>(),
    },
    touchableMeters: {
      ...(j['touchableMeters'] as List? ?? const []).cast<String>(),
    },
  ...
  factory SceneModel.fromJson(Map<String, Object?> j) => SceneModel(
    ...
    entities: [
      for (final e in (j['entities'] as List? ?? const []))
        Entity.fromJson((e as Map).cast()),
    ],
    paths: [
      for (final p in (j['paths'] as List? ?? const []))
        SolutionPath.fromJson((p as Map).cast()),
    ],
    declaredFacets: {
      ...(j['declaredFacets'] as List? ?? const []).cast<String>(),
    },
    localMeters: {
    ...
    fallbackBounds: FallbackBounds.fromJson(
      (j['fallbackBounds'] as Map).cast(),
    ),
    narrationKeys: {
      ...(j['narrationKeys'] as List? ?? const []).cast<String>(),
    },
  );
}
